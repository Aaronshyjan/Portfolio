...
export function SnakePuzzle() {
    // Generate a 15x7 grid of "activity levels" deterministically to avoid hydration mismatches
    const grid = useMemo(() => {
        return Array.from({ length: 7 }, (_, y) =>
            Array.from({ length: 20 }, (_, x) => {
                // A deterministic pseudo-random pattern based on coordinates
                const val = (x * 37 + y * 13) % 7;
                return val === 0 ? 3 : val < 3 ? 1 : val < 5 ? 2 : 0;
            })
        );
    }, []);
